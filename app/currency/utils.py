...
def get_alfabank(url):
    ...
    #INSERT 'USD' AND 'EUR'
    rate_list.insert(0, soup.find_all('h3', class_='exchange-rate-tabs__item-label')[0].get_text().replace(' ', '').replace(
        '\n', ''))
    rate_list.insert(3, soup.find_all('h3', class_='exchange-rate-tabs__item-label')[1].get_text().replace(' ', '').replace(
        '\n', ''))
